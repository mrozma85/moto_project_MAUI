...
{
    [QueryProperty(nameof(HomepageSearch), nameof(HomepageSearch))]
    public partial class ListPageTempViewModel : BaseViewModel
    {

        private readonly MotoApiService motoApiService;

        public ListPageTempViewModel(MotoApiService motoApiService)
        {
        ...
        public async Task GetAdsList()
        {
            try
            {
                if(homepageSearch != null)
                {
                    ...
                    var ads = new List<Ad>();
                    ads = await motoApiService.GetAdByName(homepageSearch);
